...
RichText richText(PaymentModel payment, dynamic property) {
  return RichText(
    ...
    const TextSpan(text: "You recieved ", style: AppTextStyles.tokenText),
    TextSpan(
        text: "₹${NumberFormat('#,##,###').format(payment.amount)} ",
        style: AppTextStyles.tokenText2),
    const TextSpan(text: "From ", style: AppTextStyles.tokenText),
...
RichText richTextTwo(PaymentModel payment, dynamic property) {
  return RichText(
      text: TextSpan(children: [
    const TextSpan(text: "You sent ", style: AppTextStyles.tokenText),
    TextSpan(
        text: "₹${NumberFormat('#,##,###').format(payment.amount)}\n",
        style: AppTextStyles.tokenText2),
    const TextSpan(text: "To ", style: AppTextStyles.tokenText),
    TextSpan(
        text: "paymen ID : ${payment.paymentId}\n",
        style: const TextStyle(color: Colors.black, fontWeight: FontWeight.w500)),
    TextSpan(
